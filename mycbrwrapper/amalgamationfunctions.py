...
class NeuralAmalgamationFunction(AmalgamationFunction):
    ...
    def __init__(self, host, name, filenames, concept):
        print("in neuralamalgamationfunction creator filenames is: {}"
              .format(filenames))
        super(NeuralAmalgamationFunction, self).__init__(host, name,
                                                         None, concept)
    ...
    def createAmalgamationFunction(self):
        ...
        api = getRequest(self.host)
        call = api.concepts(self.concept.name). \
            neuralAmalgamationFunctions(self.name)
        ...
        print("in neural createamalgamationfunction url is: {}".
              format(call._url))
        results = call.PUT(params={"type": "direct"},
                           files={"jsonfile": (jsonfilename,
                                               open(jsonfilename, 'rb'),
                                               {'Expires': '0'}),
                                  "h5file": (h5filename,
                                             open(h5filename, 'rb'),
                                             {'Expires': '0'})})
