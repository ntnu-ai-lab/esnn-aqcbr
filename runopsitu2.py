...
def plotESNNOpsitu(lr=0.2, networklayers=[50, 30, 3], dropout=0.05, epochs=200):
    model, \
    dsl, \
    train, \
    test, \
    evaler, \
    best_model,\
    losses = runevaler("opsitu", epochs, ESNNSystem, TorchEvaler,
                           eval_dual_ann, networklayers=networklayers, lr=lr,
                           dropoutrate=dropout, validate_on_k=10, filenamepostfix="esnn")
...
def plotCHOPRAOpsitu(lr=0.2, networklayers=[50, 30, 3], dropout=0.05, epochs=200):
    model, \
    dsl, \
    train, \
    test, \
    evaler, \
    best_model, \
    losses = runevaler("opsitu", epochs, ChopraTrainer, ChopraTorchEvaler,
                           eval_dual_ann, networklayers=networklayers, lr=lr,
                           dropoutrate=dropout, validate_on_k=10, filenamepostfix="chopra")
...
def plotGableOpsitu(lr=0.03, networklayers=[50, 30, 3], dropout=0.05, epochs=200):
    model, \
    dsl, \
    train, \
    test, \
    evaler, \
    best_model, \
    losses = runevaler("opsitu", epochs, GabelTrainer, GabelTorchEvaler,
              eval_gabel_ann, networklayers=networklayers,
              lr=lr, dropoutrate=dropout, validate_on_k=10,
              filenamepostfix="gabel")
...
if __name__ == "__main__":
    ...
    #choprairis()
    setLateXFonts()
    gabellosses = plotGableOpsitu(lr=0.02, networklayers=[40, 6, 3], dropout=0.005, epochs=500)
    gabeldf = pd.DataFrame(gabellosses)
    esnnlosses = plotESNNOpsitu(lr=0.06,networklayers=[40, 6, 3], dropout=0.005, epochs=500)
    esnndf = pd.DataFrame(esnnlosses)
    chopralosses = plotCHOPRAOpsitu(lr=0.06, networklayers=[40, 6, 3], dropout=0.005, epochs=500)
    ...
    alllists = []
    alllists.extend(esnnlosses)
    alllists.extend(chopralosses)
    alllists.extend(gabellosses)
    alldf = pd.DataFrame(alllists)
    plt.clf()
    plt.cla()
    plt.close()
    sns_plot = sns.lineplot(x="epoch", y="signal", hue="label", data=alldf)
    plt.show()
    fig = sns_plot.get_figure()
    fig.savefig("all.pdf")
