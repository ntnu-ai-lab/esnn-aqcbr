...
def sim_def_lin(model, test_instances, test_targets, training_instances,
    ...
    for test_instance, test_target in zip(test_instances, test_targets):
        ...
        trainingdata = zip(training_instances, training_targets)
        for training_instance, training_target in trainingdata:
            sim = global_sim_def(test_instance, training_instance,
                                 tot_maximums, tot_minmums,
                                 anynominal, nominallist)
            if sim > max_sim:
                max_sim = sim
                most_similar_case_target = training_target
        if (most_similar_case_target is not None) \
           and (np.rint(test_target) == np.rint(most_similar_case_target)).all():
            res.append(1)
        else:
            res.append(0)
    return res
    ...
    if not anynominal:
        ...
        for i in range(0, v1.shape[0]):
            if nominallist[i] is True:
                ...
                res = res + local_sim_n_def(v1[i], v2[i],
...
def global_sim_nonlin(v1, v2, maximums, minimums,
                      anynominal, nominallist, params):
    res = 0
    if not anynominal:
        for i in range(0, v1.shape[0]):
            res = res + sim_nonlin(v1[i], v2[i], params[i],
                                   maximums[i], minimums[i])
    else:
        for i in range(0, v1.shape[0]):
            if nominallist[i] is True:
